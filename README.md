...
#### SPI驱动WS2812
* 本项目通过SPI通信总线驱动WS2812。在MCU处于SPI通信的主模式时，MOSI信号线实际为受SCK时钟信号控制时序的比特流，通过调整SCK时钟信号的频率，即MCU的主频，并进行适当的分频，即可产生符合图2所示的波形。本项目使用的MCU为STM32G030K8T6，使用16MHz的HIS，通过PLL倍频和分频，设定系统的主频为50MHz。
* 在SPI配置中，设定每次8位传输，高位先发，SPI时钟16倍预分频，CPOL=0，CPHA=1，以满足WS2812的通信要求。此时SPI的波特率为3.125Mbit/s，即每一位传输需要的时间为0.32µs。在该通信协议下，WS2812的0码可表示为1000B，1码可表示为0001B，即1位数据膨胀为SPI中的4位。
* 接下来编写驱动库。先定义结构体Color_t，存储每一个LED的RGB数据。由此可以编写函数WS2812_send和WS2812_reset，WS2812_send的参数为Color_t类型的数据，用于向灯带传输一个LED灯的RGB数据，24位一起传输，从而避免了调用函数时的时间间隔导致WS2812误认成RESET码。WS2812_reset无参数，传输一段低电平，锁存信号。在这两个函数的基础上，可以编写WS2812_Handler函数。创建Color_t类型的数组ws2812_data，存储灯带192个WS2812灯的RGB数据。WS2812_Handler函数调用WS2812_send依次传输每个LED灯的数据，最后调用WS2812_reset锁存数据。根据SPI通信设置，调用一次WS2812_Handler需要的时间为：24*192*4*0.32µs+50µs = 5948.24µs至此，驱动函数编写完成，在systick中断服务程序中每10ms调用。上层用户只需要在业务逻辑程序中改变ws2812_data中的数据内容，即可实现深渊镜显示效果的变化。

#### SPI驱动WS2812（DMA方式）
* 原始的SPI驱动方案采用的是阻塞式方式，由于驱动函数在systick中断中每10ms调用一次，则CPU的时序逻辑如图，在10ms一次的中断中，SPI传输数据占用了接近6ms的时间，而在此期间，CPU只能进行轮询等待，浪费了大量的CPU时间资源；可想而知，当主程序的业务逻辑复杂时，CPU的可用资源将非常有限。
* 因此采用SPI的DMA方式进行WS2812的驱动，通过DMA将WS2812的RGB数据直接传输到SPI端口上；CPU将只在DMA开启和DMA传输完成时下达指令进行处理，其他时间都可以进行其他业务逻辑的执行，大大解放了CPU。
* 只需要在SPI配置中开启DMA通道，这里选择NORMAL，因为STM32G030MCU的RAM足够存储灯的RGB数据，因此不需要使用循环DMA来进行数据的动态膨胀。在WS2812B的库文件中开辟存储192个灯的RGB数据的数组，数组大小为192*3*4位；并编写WS2812_color_set函数，实现对指定的RGB灯的颜色进行修改。重新修改LED_Matrix库中调用WS2812B相关函数的指令，替换为WS2812_color_set，实现库移植。最后在task_Handler函数中调用新的驱动函数（实际上就是开启一次DMA传输），即可用DMA方式实现功能。
  
#### Debug
* 在驱动函数的编写过程中我们遇到了一些问题，并进行了一定程度的debug。在使用SPIDMA方式驱动WS2812时，我们发现直接在systick中断中10ms调用一次驱动函数，会使得实验现象出现异常，灯的颜色出现错误，并且更新卡顿（在轮询阻塞方式中没有这种问题）。改用1ms调用一次驱动函数，则实验效果流畅。根据逻辑分析仪的波形可知，SPI_DMA传输一次仍需要大约6ms的时间，若上一次的DMA传输还未结束，则不会开启一次新的DMA。则1ms调用一次驱动函数的数据更新密度更高。但是更根本的原因是在task_Handler函数中，除了WS2812驱动函数，还有10ms调用一次的数据更新函数，这两个函数使用了同一块内存空间，推测可能产生冲突，导致数据的乱码。因此，将两个函数的时序错位，即可解决冲突，经过实验验证准确。1ms调用一次驱动函数时，数据更新函数仍是10ms调用一次，因此它们冲突的概率大大下降。
#### 定时器DMA PWM驱动WS2812
